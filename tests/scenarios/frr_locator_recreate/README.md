...
End-to-end test that exercises the BGP-MUP locator chunk
...
the ISD/DSD from the same prefix.  The script asserts both directions
...
The script is meant to run inside the project's `vng` rootfs; the FRR
...
script -q -c "vng -m 4G --rwdir=/tmp \
  ...
  -- env PATH=$ROOT/iproute2/ip:\$PATH \
     bash $ROOT/srv6-mup-tests/tests/scenarios/frr_locator_recreate/frr_locator_recreate.sh" \
...
- `tests/scenarios/frr_only_segment/` — baseline single-locator scenario this

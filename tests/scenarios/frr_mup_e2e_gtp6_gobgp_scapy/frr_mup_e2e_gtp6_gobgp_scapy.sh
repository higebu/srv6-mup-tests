...
# tests/scenarios/frr_mup_e2e_gobgp_scapy/ but switches the UE-side and
...
#       -- ./tests/scenarios/frr_mup_e2e_gtp6_gobgp_scapy/frr_mup_e2e_gtp6_gobgp_scapy.sh
...
# docs/vpp-interop.md's existing single-hex-digit /64 slots (e/f/6/9).
#
...
# echo-request; the script sniffs for a GTP-U(v6) reply carrying the
...
then read the gnb tcpdump pcap to confirm a GTP-U(v6) reply carrying
the ICMPv6 echo-reply on the same TEID arrived back.
...
script-managed tcpdump pcap.  This script starts `tcpdump -i veth-gnb
...
if [ "$SKIP_DL" = "1" ]; then
	...
	# Send the probe and verify against the tcpdump pcap (tcpdump was
	# started earlier in the script).  We use scapy's
...
# expected to fail, and this script reports the failure cleanly so

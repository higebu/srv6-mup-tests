...
End-to-end test for the multi-VRF BGP-MUP RT-split scenario.  A
...
combination, and the script asserts that each set installs into
exactly the VRF(s) its RT(s) match.  Companion to
`tests/scenarios/frr_interop_mup` (single-VRF baseline) and
`tests/scenarios/frr_mup_e2e_gobgp_scapy` (data-plane scapy scenario).
...
as `tests/scenarios/frr_interop_mup`).

## How to run

The script uses the same vng-driven loop as the other `frr_*` test
scripts.  From the parent directory:

```
ROOT=$(cd "$(dirname "$0")/.." && pwd)  # adjust to your tree
script -q -c "vng -m 4G --run $ROOT/linux --user root \
  -- $ROOT/srv6-mup-tests/tests/scenarios/frr_mup_multi_vrf_gobgp_scapy/frr_mup_multi_vrf_gobgp_scapy.sh" \
...
The script checks the kernel install of each set's T1ST UE /32
